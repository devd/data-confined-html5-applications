;WinChan = (function() {
  ...
  function extractOrigin(url) {
    if (!/^https?:\/\//.test(url)) url = parent.location.href;
  ...
  function findRelay() {
    throw 'PRIVSEP: can only post to opener';
    var loc = parent.location;
    var frames = parent.opener.frames;
  ...
  if (isSupported()) {
    ...
    return {
      ...
      onOpen: function(cb) {
        var o = "*";
        var msgTarget = isIE ? findRelay() : __opener_post;
        ...
        function onMessage(e) {
          ...
          removeListener(window, 'message', onMessage);
          var d;
          if (e.data.__forwarded_for_winchan) {
            o = e.data.origin;
            try {

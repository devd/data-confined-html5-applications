...
function image_widget($doc_id,$doc_catg)
{
        ...
        if ( in_array($extension,$viewable_types) ) { // extention matches list
				...
				" onclick=top_restoreSession(); class='image_modal'>" .
                " <img src = $web_root" .
				"/controller.php?document&retrieve&patient_id=$pid&document_id=$doc_id" .
				...
				$to_url = "<td> <a href='" . $web_root . "/controller.php?document&retrieve" .
                    ...
                    " onclick='top_restoreSession()' class='css_button_small'>" .
                    "<span>" .
...
<a href='../reminder/active_reminder_popup.php' id='reminder_popup_link' style='visibility: false;' class='iframe' onclick='top_restoreSession()'></a>
...
<?php
 ...
 if ($thisauth == 'write') {
  ...
  if($GLOBALS['erx_enable']){
	echo '<td style="padding-left:1em;"><a class="css_button" href="../../eRx.php?page=medentry" onclick="top_restoreSession()">';
	echo "<span>".htmlspecialchars(xl('NewCrop MedEntry'),ENT_NOQUOTES)."</span></a></td>";
	echo '<td style="padding-left:1em;"><a class="css_button iframe1" href="../../soap_functions/soap_accountStatusDetails.php" onclick="top_restoreSession()">';
  ...
  if($GLOBALS['portal_onsite_enable'] && $GLOBALS['portal_onsite_address']){
    ...
    if ($portalStatus['allow_patient_portal']=='YES') {
      ...
      echo "<td style='padding-left:1em;'><a class='css_button iframe small_modal' href='create_portallogin.php?portalsite=on&patient=" . htmlspecialchars($pid,ENT_QUOTES) . "' onclick='top_restoreSession()'>";
  ...
  if($GLOBALS['portal_offsite_enable'] && $GLOBALS['portal_offsite_address']){
    ...
    if ($portalStatus['allow_patient_portal']=='YES') {
      ...
      echo "<td style='padding-left:1em;'><a class='css_button iframe small_modal' href='create_portallogin.php?portalsite=off&patient=" . htmlspecialchars($pid,ENT_QUOTES) . "' onclick='top_restoreSession()'>";
      if (empty($portalLogin)) {
...
<a href="../history/history.php" onclick='top_restoreSession()'>
...
<a href="../report/patient_report.php" onclick='top_restoreSession()'>
...
<a href="../../../controller.php?document&list&patient_id=<?php echo $pid;?>" onclick='top_restoreSession()'>
<?php echo htmlspecialchars(xl('Documents'),ENT_NOQUOTES); ?></a>
|
<a href="../transaction/transactions.php" class='iframe large_modal' onclick='top_restoreSession()'>
<?php echo htmlspecialchars(xl('Transactions'),ENT_NOQUOTES); ?></a>
|
<a href="stats_full.php?active=all" onclick='top_restoreSession()'>
...
<script language='JavaScript'>
 left_nav_setPatient(<?php echo "'" . htmlspecialchars(($result['fname']) . " " . ($result['lname']),ENT_QUOTES) .
...
?>
 ...
 ?>
 left_nav_setPatientEncounter(EncounterIdArray,EncounterDateArray,CalendarCategoryArray);
 left_nav_setRadio(window.name, 'dem');
</script>
<?php } ?>
